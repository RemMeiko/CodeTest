...
class ZhiHu {
    public static void main(String[] args) {
        //download(obtain(),"有什么美的不可方物的壁纸");
/*        Scanner in = new Scanner(System.in);
        System.out.print("请输入知乎图片网址的id:");
        String flag = in.next();
        System.out.print("请输入存储文件夹的名称:");
    ...
    // 下载网页源代码中的图片
    public static  void WebPicture() {
        // 存放所有图片的网址
        ArrayList<String> urls = new ArrayList<>();
        //访问知乎的一篇关于壁纸的回答
        String url = "https://www.zhihu.com/question/290685791/answer/474730360";
        ...
        try {
            ...
            Elements classs = document.getElementsByClass("RichContent-inner");

            Elements Divs = classs.select("figure");

            // 存储所有图片网址
            for(Element Div : Divs) {
                // 获取网址
                String Url = Div.select("img").attr("data-original");
                urls.add(Url);
            }
            System.out.println("总共有"+urls.size()+"张图片");

            // 图片名称计数
            int cnt = 1;
            for(String urlPicture : urls) {
                System.out.print("第"+cnt+"张图片网址是:"+urlPicture);
                System.out.println();
                if(urlPicture == "") {
                    System.out.println("无数据");
                }else {
                    GetOnePiture(cnt,"进击的巨人",urlPicture);
                    cnt++;
                    if(cnt == 100) {
                        break;
                    }
    ...
    public static List<String> obtain(String flag,String keyword) {
        // 存放所有图片的网址
        ArrayList<String> urls = new ArrayList<>();
        //访问知乎的一篇关于壁纸的回答
  ...
  /*          HashSet<String> set = new HashSet<>(urls);
            ...
            System.out.println("该类型总共" + (urls.size()/2) + "张图片");

            for (int i = 0;i < urls.size();i+=2) {
                System.out.print("第"+cnt+"张图片下载网址是:");
                System.out.println(urls.get(i));
                GetOnePiture(cnt,keyword,urls.get(i));
                cnt++;
            }
